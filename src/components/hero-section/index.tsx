...
export const HeroSection = () => {
  return (
    // Родительская секция: Определяет общую высоту карточки (например, 80% от экрана)
    // items-start прижимает её к верхнему краю (сразу под вашим хедером)
    <section className="relative w-full h-[80vh] bg-neutral-50 flex justify-center items-start pt-0 overflow-hidden">
      {/* Обертка КАРТИНКИ: w-[80%] делает её уже. h-full — растягивает на всю высоту родителя (80vh) */}
      {/* Закругляем только нижние углы для бесшовного соединения с хедером */}
      <div className="relative w-[80%] h-full shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] rounded-b-[40px] overflow-hidden group">
        <Image
          ...
          fill // Растягивает картинку внутри новой обертки (w-[80%] h-full)
          src="https://i.pinimg.com/1200x/86/de/25/86de25bf5b2b497bb8be816e43e60bc0.jpg"
          unoptimized
        />

        {/* Слой затемнения и блюра — теперь тоже занимает h-full */}
        ...
        {/* Контент: Используем flex-col, h-full и justify-center, чтобы все было по центру */}
        {/* Ограничим максимальную высоту контента, чтобы точно влезло */}
